...
const ResultsScreen = () => {
  ...
  const calculateOverallCGPA = () => {
    ...
    const failCount = semesters.reduce((acc, semester) => {
        if (semester.sgpa < 6) {
            return acc + 1;
        }
        return acc;
    ...
    // Determine color based on fail count
    const overallColor = failCount > 2 ? '#e74c3c' : '#008000'; // Red if more than 2 fails, green otherwise
...
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#9B59B6', // Light blue background color
    padding: 20,
  },
  semesterContainer: {
